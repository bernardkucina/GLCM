...
// COMPILE: gcc glcmSeq.c -o glcm -lm
// RUN: ./glcm --distance 1 --angles 0 --normed 1 --image 0 > test.out
int main(int argc, char *argv[])
{
    ...
    if (argc < 7) {
        printf("Usage: %s width height --distance N --angles A1 A2 … --normed B --image A\n", argv[0]);
        return 1;
    }
   ...
   if (h_imageIn == NULL)
    ...
    glcm_seq(gray_image ,histogram, angles, sum, width, height, distance, nAngles, maximum, normed);
